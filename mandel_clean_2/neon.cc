...
struct MandelbrotMultiThreaded {
  ...
  void LineRenderer (int i, EasyBMP::Image *buff) {
    using fvec_t = simde__m256;
    using ivec_t = simde__m256i;
    ...
    for (int xc = 0; xc < width; xc += 16) {
      ivec_t itr1  = simde_mm256_setzero_si256();
      ivec_t itr2  = simde_mm256_setzero_si256();
      fvec_t x1    = simde_mm256_setzero_ps();
      fvec_t x2    = simde_mm256_setzero_ps();
      fvec_t y1    = simde_mm256_setzero_ps();
      fvec_t y2    = simde_mm256_setzero_ps();
      fvec_t ab1   = simde_mm256_setzero_ps();
      fvec_t ab2   = simde_mm256_setzero_ps();

      ivec_t aCmp1 = simde_mm256_setzero_si256();
      ivec_t aCmp2 = simde_mm256_setzero_si256();
      ivec_t iCmp1 = simde_mm256_setzero_si256();
      ivec_t iCmp2 = simde_mm256_setzero_si256();
      ...
      while (cond1 | cond2) {
        auto xx1   = simde_mm256_mul_ps(x1, x1);
        auto xx2   = simde_mm256_mul_ps(x2, x2);
        auto yy1   = simde_mm256_mul_ps(y1, y1);
        auto yy2   = simde_mm256_mul_ps(y2, y2);
        auto xyn1  = simde_mm256_mul_ps(x1, y1);
        auto xyn2  = simde_mm256_mul_ps(x2, y2);
        auto xy1   = simde_mm256_add_ps(xyn1, xyn1);
        auto xy2   = simde_mm256_add_ps(xyn2, xyn2);
        auto xn1   = simde_mm256_sub_ps(xx1, yy1);
        auto xn2   = simde_mm256_sub_ps(xx2, yy2);
        ab1        = simde_mm256_add_ps(xx1, yy1);
        ab2        = simde_mm256_add_ps(xx2, yy2);
        y1         = simde_mm256_add_ps(xy1, y0);
        y2         = simde_mm256_add_ps(xy2, y0);
        x1         = simde_mm256_add_ps(xn1, x01);
        x2         = simde_mm256_add_ps(xn2, x02);
        aCmp1      = simde_mm256_castps_si256(simde_mm256_cmp_ps(ab1, simde_mm256_set1_ps(4), SIMDE_CMP_LT_OQ));
        aCmp2      = simde_mm256_castps_si256(simde_mm256_cmp_ps(ab2, simde_mm256_set1_ps(4), SIMDE_CMP_LT_OQ));
        iCmp1      = simde_mm256_cmpeq_epi32(itr1, simde_mm256_set1_epi32(max));
        iCmp2      = simde_mm256_cmpeq_epi32(itr2, simde_mm256_set1_epi32(max));
        cond1      = simde_mm256_testc_si256(iCmp1, aCmp1) == 0; 
        cond2      = simde_mm256_testc_si256(iCmp2, aCmp2) == 0;
        // only add one to the iterations of those whose ab < 4 and itr < max
        // aCmp = 1 for ab < 4
        // iCmp = 0 for itr < max
        auto inc1  = simde_mm256_andnot_si256(iCmp1, aCmp1);
        auto inc2  = simde_mm256_andnot_si256(iCmp2, aCmp2);
        // inc = -1 for (itr < max) & (ab < 4)
        // itr = itr - inc [- (-1) = + 1]
        itr1       = simde_mm256_sub_epi32(itr1, inc1);
        itr2       = simde_mm256_sub_epi32(itr2, inc2);
      }

      x01 = simde_mm256_add_ps(x01, simde_mm256_set1_ps(16*xres));
      x02 = simde_mm256_add_ps(x02, simde_mm256_set1_ps(16*xres));
      // collect results and update buffer
      int res[16] __attribute__ ((aligned(32)));
      simde_mm256_store_si256((ivec_t*)res, itr1);
      simde_mm256_store_si256((ivec_t*)(res + 8), itr2);
      for (int c = 0; c < 16; ++c) {
