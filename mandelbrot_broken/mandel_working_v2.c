...
void calc_mandel()
{
    ...
    for (i = 0; i < height; i++) {
        ...
        for (j = 0; j < width; j += 4, px += 4) {
            float32x4_t vx = vdupq_n_f32((j - (float)width/2) * scale + cx);
            float32x4_t vy = vdupq_n_f32(y);
            float32x4_t vzx = vdupq_n_f32(0);
            float32x4_t vzy = vdupq_n_f32(0);
